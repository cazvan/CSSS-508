...
```{r}
setwd("/Users/cazvan/Downloads/open_data_year_one")
folder = list.files()
folder = folder[!folder %in% "2015_status_data.csv"]
folder = folder[!folder %in% "README.txt"]
folder
```
...
```{r, cache=TRUE}
folder
data_list <- vector("list", length(folder))
```
...
[YOUR WORK]
```{r}
data_list_names = folder
data_list_names=str_sub(data_list_names, 6,-5)
```
